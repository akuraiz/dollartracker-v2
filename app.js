...
const APP_VERSION = "3.0.0-stable";
...
const I18N = {
  en: {
    ...
    whatFor:"Description", whatForPlaceholder:"Food, ABA transfer, coffee...", date:"Date", note:"Note", optionalNote:"Optional note", saveRecord:"Save Record",
  ...
  km: {
    ...
    whatFor:"ការពិពណ៌នា", whatForPlaceholder:"អាហារ, ផ្ទេរ ABA, កាហ្វេ...", date:"កាលបរិច្ឆេទ", note:"ចំណាំ", optionalNote:"ចំណាំបន្ថែម",
...
function displayToUsd(amount, currency = settings.displayCurrency) {
  return currency === "KHR" ? Number(amount || 0) / Number(settings.exchangeRate || 4000) : Number(amount || 0);
}

function formatRawMoney(value, currency) {
  const info = currencyInfo(currency);
  const formatted = Number(value || 0).toLocaleString(settings.language === "km" ? "km-KH" : undefined, {
...
function normalizeRecord(input) {
  ...
  const rate = Number(input.exchangeRate || settings.exchangeRate || 4000);
...
function filteredRecords() {
  ...
  if (searchTerm.trim()) {
    ...
    list = list.filter(r => (r.description || "").toLowerCase().includes(q) || (r.note || "").toLowerCase().includes(q) || (r.date || "").toLowerCase().includes(q) || (r.type || "").toLowerCase().includes(q));
  }
...
function translateUI() {
  ...
  setText("newTransactionTitle", tr("newTransaction")); setText("positiveOnlyText", tr("positiveOnly")); setText("typeLabel", tr("type")); setText("outLabel", tr("out")); setText("inLabel", tr("in")); setText("amountLabel", tr("amount")); setText("whatForLabel", tr("whatFor")); setText("dateLabel", tr("date")); setText("noteLabel", tr("note")); setText("saveRecordBtn", tr("saveRecord")); setText("rememberTitle", tr("remember")); setText("rememberText", tr("rememberText"), true);
...
function renderLanguageButton() {
  ...
  $("#languageFlag").src = isKhmer ? "assets/flag-kh.png" : "assets/flag-en.png";
}
...
function renderAmountChips() {
  ...
  $("#amountInput").step = info.step;
  $("#amountInput").placeholder = info.placeholder;
  ...
  target.innerHTML = list.map(record => `
    <article class="record-card">
      <div>
        ...
        <p>${displayDate(record.date)} • ${escapeHTML(record.note || (record.type === "In" ? tr("in") : tr("out")))}</p>
        ${compact ? "" : `<div class="record-actions"><button class="delete-record" type="button" data-delete="${record.id}">${tr("delete")}</button></div>`}
      </div>
...
function deleteRecord(id) {
  ...
  if (!record) return;
  if (!confirm(`${tr("deleteConfirm")}\n\n${record.type === "In" ? tr("in") : tr("out")} ${formatMoneyFromUSD(record.amountUSD)} — ${record.description || tr("noDescription")}`)) return;
...
function exportCSV() {
  const header = ["Date","Description","Type","Amount USD","Amount KHR","Net USD","Net KHR","Original Amount","Original Currency","Note"];
  const rows = sortedRecords(records).map(r => {
    const sign = r.type === "In" ? 1 : -1;
    const amountKHR = usdToDisplay(r.amountUSD, "KHR");
    return [r.date || "", r.description || "", r.type, Number(r.amountUSD || 0).toFixed(2), Math.round(amountKHR), (sign * Number(r.amountUSD || 0)).toFixed(2), Math.round(sign * amountKHR), r.originalAmount ?? "", r.originalCurrency ?? "", r.note || ""];
  });
...
function initEvents() {
  ...
  ["amountInput", "descriptionInput", "dateInput", "noteInput"].forEach(id => {
    const el = document.getElementById(id);
